...
export function PrimaryNav({ items, instanceId = "primary" }: PrimaryNavProps) {
  ...
  return (
    <nav aria-label="Primary" className="w-full">
      <ul className="relative flex items-center gap-1 overflow-x-auto rounded-full border border-[var(--border-soft)]/70 bg-[var(--color-surface-glass)] px-1 py-1 backdrop-blur-xl">
        {items.map((item) => {
